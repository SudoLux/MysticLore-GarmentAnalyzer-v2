...
export const UploadScreen: React.FC<UploadScreenProps> = ({ onImageSelected }) => {
    ...
    return (
        <div className="w-full max-w-2xl text-center animate-fade-in">
            <div className="bg-theme-card-bg backdrop-blur-xl border border-theme-glass-border p-8 sm:p-12 rounded-[20px] shadow-xl">
                <h1 className="text-3xl sm:text-4xl font-bold text-theme-text">Let's get sewing</h1>
                <p className="text-theme-text-muted mt-2">Upload a photo of a garment to begin generating your guide.</p>
                
                {isCameraActive ? (
                    <div className="mt-8">
                        <video ref={videoRef} autoPlay playsInline className="w-full rounded-lg bg-black" />
                        <div className="flex items-center justify-center gap-4 mt-4">
                            <button onClick={handleCapture} className="pill-button w-full text-white bg-theme-accent hover:opacity-90">Snap Photo</button>
                            <button onClick={stopCamera} className="pill-button w-full text-theme-text bg-theme-text/10 hover:bg-theme-text/20">Cancel</button>
                        </div>
                    </div>
                ) : (
                    <div className="mt-8 grid sm:grid-cols-2 gap-6">
                        ...
                        >
                            ...
                            <span className="text-xs text-theme-accent/80">{SUPPORTED_FORMATS_TEXT}</span>
                        </button>
                        <button
                            onClick={startCamera}
                            className="flex flex-col items-center justify-center gap-3 p-6 bg-theme-card-bg backdrop-blur-lg border border-theme-glass-border rounded-2xl text-theme-text shadow-lg transform hover:-translate-y-0.5 transition-all duration-300 hover:shadow-xl hover:border-theme-highlight dark:hover:border-theme-highlight"
                        >
                           <CameraIcon className="w-8 h-8 text-theme-highlight"/>
                           <span className="font-semibold">Use Camera</span>
                           <span className="text-xs text-theme-text-muted">For real-world items</span>
                        </button>
                    </div>
                )}
            </div>
